...
// Màu sắc cho từng depot (sử dụng hash của depot ID để tạo màu nhất quán)
const getDepotColor = (depotId: string, isActive: boolean = true) => {
  ...
  // Sử dụng hash đơn giản từ depot ID để chọn màu
  let hash = 0
  for (let i = 0; i < depotId.length; i++) {
    hash = depotId.charCodeAt(i) + ((hash << 5) - hash)
  }
  const index = Math.abs(hash) % colors.length
  return colors[index]
}

// Custom marker icon for depots với màu khác nhau
const createCustomIcon = (depotId: string, isActive: boolean = true) => {
  const color = getDepotColor(depotId, isActive)
  
  const svgIcon = `
    <svg width="32" height="45" viewBox="0 0 32 45" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <filter id="shadow-${depotId}" x="-50%" y="-50%" width="200%" height="200%">
      ...
      <path 
        d="M16 0C7.163 0 0 7.163 0 16c0 13 16 29 16 29s16-16 16-29c0-8.837-7.163-16-16-16z" 
        fill="${color}" 
        filter="url(#shadow-${depotId})"
        stroke="white"
        stroke-width="2"
      />
      <circle cx="16" cy="16" r="6" fill="white" opacity="0.9"/>
      <path 
        d="M16 10 L16 14 M16 18 L16 22 M10 16 L14 16 M18 16 L22 16" 
        stroke="white" 
        stroke-width="2" 
...
// Tọa độ mặc định cho Việt Nam (khi không có tọa độ từ API)
const DEFAULT_VIETNAM_CENTER = { lat: 16.0, lng: 106.0 }

// Hàm lấy tọa độ depot từ API hoặc sử dụng tọa độ mặc định
const getDepotCoordinates = (depot: Depot) => {
  // Ưu tiên sử dụng tọa độ từ API
  if (depot.latitude !== null && depot.latitude !== undefined && 
      depot.longitude !== null && depot.longitude !== undefined) {
    return {
      lat: depot.latitude,
      lng: depot.longitude
    }
  ...
  // Nếu có coordinates object
  if (depot.coordinates && depot.coordinates.lat && depot.coordinates.lng) {
    return {
  ...
  // Fallback: sử dụng tọa độ mặc định của Việt Nam
  console.warn(`Depot ${depot.name} không có tọa độ, sử dụng tọa độ mặc định`)
  return DEFAULT_VIETNAM_CENTER
}
  ...
  return (
    <div className="w-full h-full rounded-lg overflow-hidden relative shadow-lg">
      ...
      >
        ...
        {depots.map((depot) => {
          const coords = getDepotCoordinates(depot)
          const customIcon = createCustomIcon(depot.id, depot.status === 'active')
              ...
              <Popup className="custom-google-popup" closeButton={true} maxWidth={300}>
                <div className="p-3 min-w-[280px]">
                  ...
                  {depot.logo && depot.logo.length > 0 && depot.logo.startsWith('http') && (
                    <div className="mb-3 h-14 rounded overflow-hidden bg-white border border-gray-200">
                      <img 
                        src={depot.logo} 
                        alt={depot.name}
      ...
      <div className="absolute top-4 left-4 bg-white rounded shadow-md px-3 py-2 z-[1000] text-xs">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            <div className="w-3 h-3 rounded-full bg-gradient-to-r from-red-500 via-purple-500 to-blue-500 shadow-sm border-2 border-white"></div>
            <span className="text-gray-700">Depot hoạt động</span>
          </div>
          <div className="flex items-center gap-1.5">

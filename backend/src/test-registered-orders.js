...
const TEST_DONVIVANTAI_ID = '38512'; // Change this to test different companies
...
async function main() {
  try {
    ...
    const { token: token1, reqtime: reqtime1 } = await getToken();
    const allOrders = await getRegisteredOrders(token1, reqtime1);
    analyzeOrderData(allOrders);

    // Test 2: Get filtered orders by DonViVanTaiID
    console.log('\n\n📋 TEST 2: Get FILTERED orders by DonViVanTaiID');
    const { token: token2, reqtime: reqtime2 } = await getToken(TEST_DONVIVANTAI_ID);
    const filteredOrders = await getRegisteredOrders(token2, reqtime2, TEST_DONVIVANTAI_ID);
    analyzeOrderData(filteredOrders);

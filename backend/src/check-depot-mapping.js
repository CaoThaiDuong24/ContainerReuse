...
async function checkDepotMapping() {
  ...
  try {
    ...
    depots.forEach((depot, index) => {
      ...
      const depotName = depot.TenDepot?.v || depot.TenDepot?.r;
      const depotCode = depot.MaDepot?.v || depot.MaDepot?.r;
      const shortName = depot.TenVietTat?.v || depot.TenVietTat?.r;
      
      console.log(`\n${index + 1}. ${depotName}`);
      console.log(`   ID: ${depotId || '❌ NO ID'}`);
      console.log(`   Code: ${depotCode || 'N/A'}`);
      console.log(`   Short Name: ${shortName || 'N/A'}`);
      
      if (!depotId) {
        console.log('   ⚠️ WARNING: This depot has no ID!');
      }
    });

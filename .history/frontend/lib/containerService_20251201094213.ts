...
export async function fetchContainersByDepotId(depotId: string): Promise<ApiResponse<Container[]>> {
  try {
    console.log('[ContainerService] Fetching containers for depot:', depotId);
    ...
    console.log('[ContainerService] Depot IDs in data:', [...new Set(response.data.map(c => c.depotId))]);
    
    // Filter containers by depotId on client side
    const filteredContainers = response.data.filter(
      (container: Container) => {
        // Try exact match first
        if (container.depotId === depotId) return true;
        // Try string comparison
        if (String(container.depotId) === String(depotId)) return true;
        return false;
      }
    );

    console.log('[ContainerService] Filtered containers:', filteredContainers.length);

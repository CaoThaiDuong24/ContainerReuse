 ...
 * 
 * ⚠️ WARNING: Backend HRMS_UserProfile API currently returns empty array
 * This service will work once the API has data, or when switched to
 * alternative data source (e.g., extracting from driver API)
 */
...
/**
 * Fetch company by user ID (accuserkey)
 */
export async function getCompanyByUserId(accuserkey: string): Promise<SingleCompanyApiResponse> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/companies/by-user/${accuserkey}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'

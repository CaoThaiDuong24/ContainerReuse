...
export async function fetchContainersByDepotId(depotId: string): Promise<ApiResponse<Container[]>> {
  try {
    ...
    // Log all unique depot IDs
    const uniqueDepotIds = [...new Set(response.data.map(c => c.depotId))];
    console.log('[ContainerService] All unique depot IDs in data:', uniqueDepotIds);
    
    // Log first few containers with their depotIds
    console.log('[ContainerService] Sample containers (first 5):');
    response.data.slice(0, 5).forEach((c: Container) => {
      console.log(`  - Container ${c.containerId}: depotId="${c.depotId}" (type: ${typeof c.depotId}), depotName="${c.depotName}"`);
    });

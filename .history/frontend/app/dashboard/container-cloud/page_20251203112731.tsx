                  ...
                  {loading ? (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      const shippingLinesGrouped = depotContainers.reduce((acc, container) => {
                        const shipLine = shippingLines.find(sl => sl.code === container.owner || sl.name === container.owner);
                        const key = container.owner;
                        if (!acc[key]) {
                          acc[key] = { 
                            count: 0, 
                            logo: shipLine?.logo || '',
                            color: shipLine?.colorTemplate || ''
                          };
                        }
                        acc[key].count += 1;
                        return acc
                      }, {} as Record<string, { count: number, logo: string, color: string }>)
                            ...
                            {Object.keys(shippingLinesGrouped).length > 0 ? (
                              <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {Object.entries(shippingLinesGrouped).map(([line, data]) => (
                                    <div 
                                      key={line}
                                      className="flex items-center gap-1.5 bg-white border rounded-md px-2 py-1 shadow-sm"
                                    ...
                                    >
                                      {data.logo ? (
                                        <div className="w-5 h-5 rounded overflow-hidden flex items-center justify-center flex-shrink-0">
                                          <img 
                                            src={`https://cms.ltacv.com${data.logo}`}
                                            alt={line}
                                      ...
                                      <span className="text-xs font-bold text-blue-600">{data.count}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>

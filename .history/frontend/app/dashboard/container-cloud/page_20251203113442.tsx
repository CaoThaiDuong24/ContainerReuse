...
export default function ContainerCloudPage() {
  ...
  // Filter depots for map and list display - based on province AND selected depot
  const filteredDepots = useMemo(() => {
    ...
    // Filter by specific depot if selected
    if (selectedDepot !== "all") {
      depotsToShow = depotsToShow.filter(d => d.id === selectedDepot)
    }
    
    return depotsToShow
  }, [depots, selectedProvince, selectedDepot])

  // Get containers for depot display - ONLY filtered by province and depot
  // This is what we show in the depot cards - should NOT be affected by shipping line/size/type filters
  const containersForDepotDisplay = useMemo(() => {
    let filtered = containers
  ...
  // Filter shipping lines based on containers in DEPOT-filtered containers (not affected by shipping line filter itself)
  const availableShippingLines = useMemo(() => {
    const uniqueOwners = Array.from(new Set(containersForDepotDisplay.map(c => c.owner).filter(Boolean)))
    return uniqueOwners.sort()
  }, [containersForDepotDisplay])
                ...
                <div className="p-3">
                  {loading ? (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      // Use containersForDepotDisplay instead of filteredContainers
                      // This shows ALL containers in the depot, not filtered by shipping line/size/type
                      const depotContainers = containersForDepotDisplay.filter(c => c.depotId === depot.id)

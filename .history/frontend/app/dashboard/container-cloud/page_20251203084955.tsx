...
export default function ContainerCloudPage() {
  ...
  const filteredDepots = useMemo(() => {
    // First, filter containers without depot filter to get available depots
    let containersForDepotFilter = containers

    // Filter by province (through depot)
    if (selectedProvince !== "all") {
      const provinceDepots = depots.filter(d => d.province === selectedProvince)
      const depotIds = provinceDepots.map(d => d.id)
      containersForDepotFilter = containersForDepotFilter.filter(c => depotIds.includes(c.depotId))
    }

    // Filter by shipping line
    if (selectedShippingLine !== "all") {
      containersForDepotFilter = containersForDepotFilter.filter(c => c.owner === selectedShippingLine)
    }

    // Filter by size
    if (selectedSize !== "all") {
      containersForDepotFilter = containersForDepotFilter.filter(c => c.size === selectedSize)
    }

    // Filter by type
    if (selectedType !== "all") {
      containersForDepotFilter = containersForDepotFilter.filter(c => c.type === selectedType)
    }

    // Search filter
    if (searchTerm) {
      containersForDepotFilter = containersForDepotFilter.filter(c => 
        c.containerId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.depotName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.owner.toLowerCase().includes(searchTerm.toLowerCase())
      )
    ...
    // Get unique depot IDs from filtered containers (without depot filter)
    const depotIdsWithContainers = Array.from(new Set(containersForDepotFilter.map(c => c.depotId)))
    ...
    return depotsToShow
  }, [containers, depots, selectedProvince, selectedShippingLine, selectedSize, selectedType, searchTerm])

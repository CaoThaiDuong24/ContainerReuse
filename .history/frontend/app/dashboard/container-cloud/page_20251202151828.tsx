                        ...
                        {filteredContainers.map((container) => {
                          const getStatusColor = (status: string) => {
                            switch (status) {
                              ...
                              case 'in-use': return 'bg-blue-50 border-blue-200'
                              case 'maintenance': return 'bg-amber-50 border-amber-200'
                              case 'reserved': return 'bg-purple-50 border-purple-200'
                          ...
                          const getStatusBadge = (status: string) => {
                            switch (status) {
                              case 'available': return { label: 'Khả dụng', className: 'bg-green-500 text-white' }
                              case 'in-use': return { label: 'Đang dùng', className: 'bg-blue-500 text-white' }
                              case 'maintenance': return { label: 'Bảo trì', className: 'bg-amber-500 text-white' }
                              case 'reserved': return { label: 'Đã đặt', className: 'bg-purple-500 text-white' }
                              default: return { label: status, className: 'bg-gray-500 text-white' }
                            }
                          }
                          ...
                          return (
                            <Card
                              key={container.id}
                              className={`group hover:shadow-lg transition-all duration-300 cursor-pointer border-2 ${getStatusColor(container.status)} hover:scale-[1.02] hover:border-blue-400`}
                            >
                              <CardContent className="p-4">
                                {/* Header: Container ID & Status */}
                                <div className="flex items-start justify-between mb-3 pb-3 border-b-2 border-dashed border-gray-200">
                                  <div className="flex items-center gap-2 flex-1 min-w-0">
                                    <div className="p-2 bg-blue-600 rounded-lg group-hover:bg-blue-700 transition-colors">
                                      <ContainerIcon className="h-4 w-4 text-white" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <div className="flex items-center gap-1 mt-0.5">
                                        <Badge className={`text-[10px] px-1.5 py-0 h-4 ${getStatusBadge(container.status).className}`}>
                                          {getStatusBadge(container.status).label}
                                        </Badge>
                                      </div>
                                    </div>
                                  </div>

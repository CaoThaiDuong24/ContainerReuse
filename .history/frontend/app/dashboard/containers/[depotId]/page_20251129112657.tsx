...
import { Container, getContainersByDepotId, Depot } from "@/lib/mockData"
import { fetchDepotById } from "@/lib/depotService"
  ...
  useEffect(() => {
    if (depotId) {
      ...
      const loadDepotData = async () => {
        const response = await fetchDepotById(depotId)
        if (response.success && response.data) {
          setDepot(response.data)
        } else {
          console.error('Failed to load depot:', response.error)
        }
      }
      loadDepotData()

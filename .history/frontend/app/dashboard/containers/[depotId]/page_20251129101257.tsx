        ...
        <div className="w-full p-4 md:p-6 lg:p-8 space-y-6">
          ...
          <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {isLoading ? (
              ...
              <Card className="border-none shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 bg-white dark:bg-slate-900 hover:shadow-xl transition-shadow duration-300 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <CardContent className="p-6">
          ...
          {/* Filters & Search */}
          <div className="sticky top-4 z-30 animate-in fade-in slide-in-from-top-4 duration-500">
            <Card className="border-none shadow-lg shadow-slate-200/50 dark:shadow-none bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl supports-[backdrop-filter]:bg-white/70 hover:shadow-xl transition-shadow">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
                    <Select value={filterStatus} onValueChange={setFilterStatus}>
                      <SelectTrigger className="w-[160px] bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 hover:border-slate-300 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 animate-in fade-in slide-in-from-bottom-6 duration-700">
              {filteredContainers.length === 0 ? (
                ...
                filteredContainers.map((container, index) => (
                  <Card 
                    key={container.id} 
                    className="group hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden cursor-pointer animate-in fade-in slide-in-from-bottom-4"
                    style={{ animationDelay: `${index * 50}ms` }}
                  >
                    <CardContent className="p-0">
                      {/* Card Header */}
                      <div className="p-5 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-br from-slate-50/80 via-slate-50/50 to-white dark:from-slate-900/80 dark:via-slate-900/50 dark:to-slate-900 group-hover:from-slate-100 dark:group-hover:from-slate-800 transition-colors">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all">
                              <ContainerIcon className="h-5 w-5 text-blue-600 dark:text-blue-500" />
                            </div>
                            <div>
                              <h3 className="font-bold text-lg text-slate-900 dark:text-slate-100 tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                {container.containerId}
                              </h3>
                              <p className="text-xs text-muted-foreground font-medium">{container.owner}</p>
                            </div>
                          </div>
                          <Badge variant="outline" className={`${getStatusColor(container.status)} border flex items-center gap-1 transition-all`}>
                        ...
                        <div className="flex gap-2">
                          <Badge variant="secondary" className="bg-slate-200/70 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                            {container.size}
                          </Badge>
                          <Badge variant="secondary" className="bg-slate-200/70 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                      ...
                      <div className="p-5 space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <div className="flex items-center gap-1.5 text-muted-foreground">
                              <MapPin className="h-3.5 w-3.5" />
                              <span className="text-xs font-medium uppercase tracking-wide">Vị trí</span>
                            </div>
                          ...
                          <div className="space-y-1.5 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <div className="flex items-center gap-1.5 text-muted-foreground">
                              ...
                              <span className="text-xs font-medium uppercase tracking-wide">Ngày nhập</span>
                            </div>
                            <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 pl-5">
                        ...
                        {container.estimatedOutDate && (
                          <div className="p-3 rounded-lg bg-gradient-to-br from-orange-50 to-orange-100/50 dark:from-orange-950/30 dark:to-orange-950/10 border border-orange-200 dark:border-orange-900/30 flex items-center gap-3 hover:shadow-md transition-shadow">
                            <div className="p-2 bg-white dark:bg-orange-900/20 rounded-lg">
                              <Clock className="h-4 w-4 text-orange-600 dark:text-orange-400" />
                            </div>
                            <div className="flex-1">
                              <p className="text-xs text-orange-600/80 dark:text-orange-400/80 font-medium uppercase tracking-wide">Dự kiến xuất</p>
                        ...
                        <div className="flex gap-3 pt-2">
                          {container.status === 'available' ? (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-md shadow-blue-600/30 hover:shadow-lg hover:shadow-blue-600/40 transition-all">
                            ...
                            <Button disabled variant="secondary" className="flex-1 opacity-60 cursor-not-allowed">
                              Không khả dụng
                            </Button>
                          )}
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="outline" size="icon" className="shrink-0 border-slate-200 dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-blue-950/20 hover:border-blue-300 dark:hover:border-blue-700 transition-all group/btn">
                                <Eye className="h-4 w-4 text-slate-500 group-hover/btn:text-blue-600 dark:group-hover/btn:text-blue-400 transition-colors" />
                              </Button>
                            </TooltipTrigger>
                            <TooltipContent>
                              <p>Xem chi tiết</p>

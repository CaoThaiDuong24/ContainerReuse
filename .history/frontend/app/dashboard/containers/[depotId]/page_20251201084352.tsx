...
import { fetchReuseContainers, transformApiDataToContainers } from "@/lib/containerService"
...
export default function DepotContainersPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const containerResponse = await fetchReuseContainers()
      if (containerResponse.success) {
        const transformedContainers = transformApiDataToContainers(containerResponse.data)
        
        // Filter containers by depotId
        const depotContainers = transformedContainers.filter(c => c.depotId === depotId)
        setContainers(depotContainers)
        setFilteredContainers(depotContainers)
        
        console.log(`Loaded ${depotContainers.length} containers for depot ${depotId}`)
      } else {
        setError(containerResponse.message || 'Không thể tải dữ liệu container')
      }
    } catch (err) {

        ...
        {!loading && !error && filteredContainers.length > 0 && (
          <Card className="border-purple-200">
            <CardHeader>
              ...
              <CardDescription>
                Hiển thị {paginatedContainers.length} container (trang {currentPage} / {totalPages})
              </CardDescription>
            </CardHeader>
            <CardContent>
              {/* Desktop Table */}
              <div className="hidden md:block rounded-lg border border-purple-200 overflow-hidden">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gradient-to-r from-purple-50 to-blue-50 hover:from-purple-50 hover:to-blue-50">
                      <TableHead className="font-semibold text-gray-900">Số Container</TableHead>
                      <TableHead className="font-semibold text-gray-900">Số Booking</TableHead>
                      <TableHead className="font-semibold text-gray-900">Loại/Kích thước</TableHead>
                      <TableHead className="font-semibold text-gray-900">Trạng thái</TableHead>
                      <TableHead className="font-semibold text-gray-900">Depot</TableHead>
                      <TableHead className="font-semibold text-gray-900">Số xe</TableHead>
                      ...
                      >
                        <TableCell className="font-medium">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-purple-700">{container.containerId || 'N/A'}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex flex-col">
                            <span className="font-medium text-gray-900">{container.type || 'N/A'}</span>
                            {container.size && (
              ...
              <div className="md:hidden space-y-3">
                {paginatedContainers.map((container) => (
                  <Card key={container.id} className="border-purple-200 hover:shadow-md transition-shadow">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="font-bold text-purple-700">{container.containerId || 'N/A'}</span>
                            </div>
                            <div className="text-sm text-blue-600 font-medium mb-2">
                              Booking: {container.containerNumber || 'N/A'}
                            </div>
                          </div>
                          <Badge className={getStatusColor(container.status)}>
                            {container.status}
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            ...
                            <p className="font-medium text-gray-900">{container.type || 'N/A'}</p>
                          </div>
                          <div>
                            <span className="text-gray-500 text-xs">Kích thước:</span>
                            <p className="font-medium text-gray-900">{container.size || 'N/A'}</p>
                          </div>
                          <div>
                            <span className="text-gray-500 text-xs">Depot:</span>
                            <div className="flex items-center gap-1">
                              <MapPin className="h-3 w-3 text-gray-400" />
                              <p className="font-medium text-gray-900">{container.depot || '-'}</p>
                            </div>
                          </div>
                          <div>
                            <span className="text-gray-500 text-xs">Số xe:</span>
                            <div className="flex items-center gap-1">
                              <Truck className="h-3 w-3 text-gray-400" />
                              <p className="font-medium text-gray-900">{container.vehicleNumber || '-'}</p>
                            </div>
                          </div>
                        </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-purple-200">
                  <div className="text-sm text-gray-600">
                    Hiển thị {((currentPage - 1) * itemsPerPage) + 1} - {Math.min(currentPage * itemsPerPage, filteredContainers.length)} trong tổng số {filteredContainers.length} container
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"

...
import { Download, PackageCheck, FileText, User, Truck, DollarSign, UserCheck, Building2 } from "lucide-react"
import { Container } from "@/lib/containerService"
import { Driver, getDriversByCompany } from "@/lib/driverService"
import { Company, getAllCompanies, getCompanyByUserId } from "@/lib/companyService"
...
export function PickupContainerModal({ 
    ...
    const fetchUserCompany = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          console.log(`✅ Loaded user company from HRMS_UserProfile:`, result.data)
          
          // Auto-fill company ID
          setFormData(prev => ({
            ...prev,
            transportCompanyId: result.data.id,
            invoiceCompanyInfra: result.data.id,
            invoiceCompany: result.data.id,
          }))
  ...
  // Fetch companies when modal opens (for backup/reference)
  useEffect(() => {
    const fetchCompanies = async () => {
      ...
      try {
        ...
        if (result.data && result.data.length > 0) {
          ...
          console.warn('⚠️ HRMS_UserProfile API returned 0 companies')
          console.warn('💡 Fallback: Users can manually enter transport company ID')
        }
      } catch (error) {
        console.error('Error fetching companies:', error)
        console.warn('⚠️ Company API failed, using manual input mode')
        setCompanies([])
      } finally {
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {/* Chọn công ty/nhà xe */}
                <div className="flex flex-col">
                  <Label htmlFor="companySelect" className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-1.5 mb-2">
                    <Building2 className="h-4 w-4 text-purple-500 shrink-0" />
                    <span>Công ty/Nhà xe</span>
                    {userCompany && (
                      <span className="text-xs text-green-600 dark:text-green-400 ml-1">(Tự động)</span>
                    )}
                  </Label>
                  {loadingUserCompany ? (
                    <div className="h-11 flex items-center px-3 bg-slate-50 dark:bg-slate-800 rounded-md border">
                      <span className="text-sm text-slate-500">Đang tải thông tin công ty...</span>
                    </div>
                  ) : userCompany ? (
                    <div className="h-auto min-h-[44px] flex flex-col gap-1 p-3 bg-green-50 dark:bg-green-900/20 rounded-md border border-green-200 dark:border-green-800">
                      <div className="flex items-center gap-2">
                        <Building2 className="h-4 w-4 text-green-600 dark:text-green-400 shrink-0" />
                        <span className="text-sm font-medium text-green-900 dark:text-green-100">{userCompany.name}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-green-700 dark:text-green-300">
                        <span>Mã: {userCompany.id}</span>
                        {userCompany.rawApiData?.source && (
                          <span className="px-1.5 py-0.5 bg-green-100 dark:bg-green-900/40 rounded text-[10px]">
                            {userCompany.rawApiData.source === 'HRMS_UserProfile' ? 'HRMS' : 'Driver API'}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => {
                        ...
                        setFormData({
                          ...
                          // Auto-fill invoice companies with the same ID
                          invoiceCompanyInfra: value,
                          invoiceCompany: value
                        });
                        if (company) {
                          toast.success(`Đã chọn: ${company.name}`);
                        }
                      }}
                      disabled={loadingCompanies || companies.length === 0}
                    >
                      <SelectTrigger className="h-11 text-sm focus:ring-2 focus:ring-purple-500">
                        ...
                        {companies.map((company) => (
                          <SelectItem key={company.id} value={company.id}>
                            <div className="flex flex-col">
                              ...
                              {company.code && (
                                <span className="text-xs text-slate-500">Mã: {company.code}</span>
                              )}
                            </div>
                          </SelectItem>
                        ))}
                  ...
                  {loadingUserCompany && (
                    <p className="text-xs text-blue-500 mt-1 flex items-center gap-1">
                      ...
                      Đang lấy thông tin từ HRMS_UserProfile...
                    </p>
                  )}
                  {!loadingUserCompany && !userCompany && loadingCompanies && (
                    <p className="text-xs text-purple-500 mt-1">Đang tải danh sách công ty...</p>
                  )}
                  {!loadingUserCompany && !userCompany && !loadingCompanies && companies.length === 0 && (
                    <p className="text-xs text-amber-600 mt-1">
                      ⚠️ Danh sách công ty chưa khả dụng. Vui lòng nhập mã thủ công bên dưới.
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <Input 
                    ...
                    title={companies.length > 0 ? "Mã tự động điền khi chọn công ty" : "Nhập mã đơn vị vận tải"}
                  />
                  {companies.length === 0 && (
                    <p className="text-xs text-blue-500 mt-1">
                      💡 Nhập mã đơn vị vận tải (bắt buộc)
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <Input 
                    ...
                    title="Tự động điền khi chọn công ty"
                  />
                  {formData.invoiceCompanyInfra && formData.transportCompanyId === formData.invoiceCompanyInfra && (
                    <p className="text-xs text-purple-600 mt-1">✓ Tự động điền từ công ty đã chọn</p>
                  )}
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <Input 
                    ...
                    title="Tự động điền khi chọn công ty"
                  />
                  {formData.invoiceCompany && formData.transportCompanyId === formData.invoiceCompany && (
                    <p className="text-xs text-purple-600 mt-1">✓ Tự động điền từ công ty đã chọn</p>
                  )}
                </div>

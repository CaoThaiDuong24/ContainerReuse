...
import { Company, getAllCompanies } from "@/lib/companyService"
    ...
    const fetchCompanies = async () => {
      ...
      try {
        ...
        if (result.data && result.data.length > 0) {
          ...
          // Don't show error toast - just use manual input as fallback
        }
      } catch (error) {
        console.error('Error fetching companies:', error)
        console.warn('⚠️ Company API failed, using manual input mode')
        setCompanies([])
        // Don't show error toast - gracefully degrade to manual input
      } finally {

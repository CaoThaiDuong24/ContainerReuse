...
async function testHRMSUserProfile() {
  ...
  try {
    ...
    if (allResponseData.data && allResponseData.data.length > 0) {
      console.log('\n' + '='.repeat(80));
      console.log('📋 SAMPLE RECORD (First item):');
      console.log('='.repeat(80));
      
      const firstRecord = allResponseData.data[0];
      ...
      if (allResponseData.data.length > 1) {
        console.log('\n' + '='.repeat(80));
        console.log(`📋 SHOWING ${Math.min(5, allResponseData.data.length)} SAMPLE RECORDS:`);
        console.log('='.repeat(80));
        
        for (let i = 0; i < Math.min(5, allResponseData.data.length); i++) {
          const record = allResponseData.data[i];
          console.log(`\nRecord ${i + 1}:`);
      ...
      console.log('\nFull Response:');
      console.log(JSON.stringify(allResponseData, null, 2));
    }

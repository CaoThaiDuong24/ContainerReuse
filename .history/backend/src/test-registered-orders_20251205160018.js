    ...
    if (response.data) {
      ...
      if (response.data.data && Array.isArray(response.data.data)) {
        ...
        if (response.data.data.length > 0) {
          ...
          // Show last 3 orders
          console.log('\n📋 Last 3 orders:');
          const lastOrders = response.data.data.slice(-3);
          lastOrders.forEach((order, idx) => {
            console.log(`\n  Order ${response.data.data.length - 3 + idx + 1}:`);
            console.log(`    ID: ${order.ID?.v || order.ID || 'N/A'}`);
            console.log(`    Container: ${order.SoChungTuNhapBai?.v || order.ContID?.v || 'N/A'}`);
            console.log(`    Type: ${order.ContainerType?.v || order.LoaiCont?.v || 'N/A'}`);
            console.log(`    Size: ${order.ContainerSize?.v || order.KichThuoc?.v || 'N/A'}`);
            console.log(`    Depot: ${order.DepotName?.v || order.Depot?.v || 'N/A'}`);
            console.log(`    Company ID: ${order.DonViVanTaiID?.v || order.DonViVanTaiID || 'N/A'}`);
            console.log(`    Created: ${order.NgayTao?.v || order.ThoiGianTao?.v || 'N/A'}`);
          });
        }
        ...
        const depotMap = {};
        response.data.data.forEach(order => {
          const depot = order.DepotName?.v || order.Depot?.v || 'Unknown';
          depotMap[depot] = (depotMap[depot] || 0) + 1;
        });
        Object.entries(depotMap).forEach(([depot, count]) => {
          console.log(`  ${depot}: ${count} orders`);
        });

  ...
  private transformContainerTypeData(apiData: any): ContainerType[] {
    try {
      ...
      return rawData.map((item: any, index: number) => {
        ...
        const id = getValue(item.ID || item.id || item.Id);
        const name = getValue(item.TenLoaiCont || item.Ten || item.Name || item.name);
        const code = getValue(item.MaLoaiCont || item.Ma || item.Code || item.code);
        const description = getValue(item.MoTa || item.Description || item.description);
        const status = getValue(item.TrangThai || item.Status || item.status);
        ...
        return {
          ...
          status: (status === '1' || status.toLowerCase() === 'active' || status === 'Hoạt động') ? 'active' : 'inactive',
          createdAt: getValue(item.NgayTao || item.CreatedAt) || undefined,
          updatedAt: getValue(item.NgayCapNhat || item.UpdatedAt) || undefined,
        };
      });
    } catch (error) {

...
class CompanyApiService {
   ...
   * Note: API doesn't support AccountID filter, so we get all data and filter client-side
   */
    ...
    try {
      ...
      if (response.data) {
        if (response.data.data && Array.isArray(response.data.data)) {
          ...
          // Filter client-side by AccountID
          if (response.data.data.length > 0) {
            ...
            const filtered = response.data.data.filter((profile: any) => {
              ...
              return id === accountId || id === String(accountId);
            });

            if (filtered.length > 0) {
              console.log(`✅ Found profile for AccountID ${accountId}`);
              return { ...response.data, data: filtered };
            } else {
              console.log(`⚠️ No profile found for AccountID ${accountId} in ${response.data.data.length} records`);
            }
          }
        }
  ...
  /**
   * Get company by user accuserkey
   * 1. First tries HRMS_UserProfile API with AccountID parameter
  ...
  async fetchCompanyByUserId(accuserkey: string): Promise<Company | null> {
    try {
      ...
      const profileData = await this.getUserProfileData(accuserkey);
      ...
      // Find driver with matching accuserkey
      const userDriver = driverData.data.find((driver: any) => {
        const driverId = getValue(driver.ID_driver);
        return driverId === accuserkey;
      });

      if (!userDriver) {
        console.warn(`⚠️ No data found for user: ${accuserkey}`);
        return null;
      }

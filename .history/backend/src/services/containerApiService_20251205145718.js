...
class ContainerApiService {
  ...
  async getListDonHangReUseOutNow(companyId = null) {
    try {
      ...
      // We will filter the results after receiving them
      console.log('payload', {
      ...
      if (response.data && response.data.data) {
        ...
        if (companyId) {
          ...
          containers = containers.filter(c => 
            c.companyId && c.companyId.toString() === companyIdStr
          );
          console.log(`📊 Filtered containers for company ${companyId}:`, containers.length);
        }
      ...
      if (error.response && (error.response.status === 401 || error.response.status === 403)) {
        ...
        if (tokenData) {
          this.token = tokenData.token;
          this.reqtime = tokenData.reqtime;
          return await this.getListDonHangReUseOutNow(companyId);
        }
      }
      return [];
    }

...
class CompanyApiService {
  ...
  async fetchCompanyByUserId(userId: string): Promise<Company | null> {
    try {
      ...
      if (userProfileData && userProfileData.data && Array.isArray(userProfileData.data)) {
        ...
        // Find user profile with matching user ID
        const userProfile = userProfileData.data.find((profile: any) => {
          const profileUserId = getValue(profile.UserID) || getValue(profile.ID_driver) || getValue(profile.AccUserKey);
          return profileUserId === userId;
        });

        if (userProfile) {
          const companyId = getValue(userProfile.CompanyID) || getValue(userProfile.NhaXeID);
    ...
    try {
      ...
      apiData.data.forEach((profile: any) => {
        ...
        if (companyId && !companyMap.has(companyId)) {
          const companyName = getValue(profile.CompanyName) || getValue(profile.TenNhaXe) || `Nhà xe ${companyId}`;

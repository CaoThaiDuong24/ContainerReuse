    ...
    try {
      ...
      if (response.data) {
        if (response.data.data && Array.isArray(response.data.data)) {
          ...
          // Filter client-side by ID (checking ID, AccountID, UserID fields - prioritize ID)
          if (response.data.data.length > 0) {
            ...
            const filtered = response.data.data.filter((profile: any) => {
              // Check ID first, then AccountID, then UserID
    ...
    try {
      ...
      if (profileData && profileData.data) {
        ...
        if (profile) {
          ...
          // Use ID from HRMS_UserProfile as the primary company ID
          const companyId = getValue(profile.ID) || getValue(profile.CompanyID) || getValue(profile.NhaXeID);

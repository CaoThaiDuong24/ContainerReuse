...
class ContainerApiService {
  ...
  async transformContainerData(apiData) {
    ...
    return apiData.map(item => {
      ...
      const typeMap = {
        'GP': 'GP',  // General Purpose - Khô
        'HC': 'HC',  // High Cube - Cao
        'RF': 'RF',  // Reefer - Lạnh
        'UT': 'UT',  // Open Top - Nóc mở
        'OT': 'UT',  // Open Top alternative
        'PC': 'PC',  // Flat Collapsible - Phẳng
        'PF': 'PF',  // Flat Fixed - Phẳng cố định
        'FR': 'PC',  // Flatrack alternative
        'TN': 'TN',  // Tank - Bồn
        'TK': 'TN'   // Tank alternative
      };
      const type = typeMap[item.ContainerType?.v] || 'GP';

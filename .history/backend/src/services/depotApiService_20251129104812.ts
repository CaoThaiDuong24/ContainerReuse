  ...
  transformDepotData(apiData: any): any[] {
    try {
      ...
      return depots.map((depot: any) => {
        ...
        const depotId = getValue(depot.ID) || getValue(depot.DepotID);
        ...
        return {
          id: depotId || `DEPOT${getValue(depot.SEQ)}`,
          name: name,
          ...
          province: this.getCityName(getValue(depot.ThanhPho)),
          // Thêm các field bổ sung
          tenVietTat: shortName,
          dienTich: getValue(depot.DienTich),
          depotID: getValue(depot.DepotID),
          active_eDepot: getValue(depot.Active_eDepot) === 'True'
        };
      }).filter(depot => depot.status === 'active'); // Chỉ lấy depot active
    } catch (error) {

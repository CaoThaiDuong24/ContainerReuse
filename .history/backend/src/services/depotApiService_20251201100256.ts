  ...
  transformDepotData(apiData: any): any[] {
    try {
      ...
      return depots.map((depot: any) => {
        const depotId = getValue(depot.ID);
        const depotIDField = getValue(depot.DepotID);
        // Ưu tiên ID trước, nếu không có (undefined/null) thì dùng DepotID
        // Lưu ý: ID có thể là "0" (string) nên phải check !== undefined
        const finalDepotId = depotId !== undefined && depotId !== null && depotId !== '' 
          ? depotId 
          : depotIDField;
        ...
        return {
          id: finalDepotId, // Dùng ID từ API (có thể là "0", "1", "15", etc.)
          name: name,

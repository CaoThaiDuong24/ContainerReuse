...
class CompanyApiService {
   ...
   * Finds the company that the user belongs to by matching driver ID
   */
  async fetchCompanyByUserId(accuserkey: string): Promise<Company | null> {
    try {
      console.log(`🔍 Finding company for user: ${accuserkey}`);
      ...
      const userDriver = rawData.data.find((driver: any) => {
        const driverId = getValue(driver.ID_driver);
        return driverId === accuserkey;
      });

      if (!userDriver) {
        console.warn(`⚠️ No driver found with accuserkey: ${accuserkey}`);
        return null;
      }
      ...
      if (!nhaXeID) {
        console.warn(`⚠️ Driver ${accuserkey} has no company assigned`);
        return null;
      }

      console.log(`✅ Found company ${nhaXeID} for user ${accuserkey}`);

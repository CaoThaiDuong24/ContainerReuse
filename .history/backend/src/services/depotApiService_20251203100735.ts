  ...
  transformDepotData(apiData: any): any[] {
    try {
      ...
      return depots.map((depot: any) => {
        ...
        // Logo từ API - nhưng các file không tồn tại trên server (404)
        // Tạm thời không sử dụng logo từ API do lỗi 404
        const logoPath = getValue(depot.logo_inform_whm);
        // const logoUrl = logoPath ? `http://apiedepottest.gsotgroup.vn${logoPath}` : '';
        const logoUrl = ''; // Tạm thời disable vì tất cả logo đều 404
        ...
        return {
          ...
          address: address,
          image: 'https://images.unsplash.com/photo-1566576721346-d4a3b4eaeb55?w=800&q=80', // Default image for containers page
          logo: logoUrl, // Empty - logos from API return 404

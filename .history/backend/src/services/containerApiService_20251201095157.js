...
class ContainerApiService {
  ...
  async transformContainerData(apiData) {
    ...
    // Get depot ID mapping from real depot API
    const depotIdMap = await this.getDepotIdMapping();
    console.log('📍 Using depot mapping:', depotIdMap);
    ...
    return apiData.map(item => {
      ...
      // Map to frontend depot ID format using real depot data
      // depotIdMap is loaded from depot API (e.g., depot ID 15 → DEPOT5 based on SEQ)

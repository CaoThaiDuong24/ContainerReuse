  ...
  transformDepotData(apiData: any): any[] {
    try {
      ...
      return depots.map((depot: any) => {
        ...
        // Lấy logo từ API - chỉ lấy khi có logo thật, không dùng ảnh mặc định
        const logoPath = getValue(depot.logo_inform_whm);
        const logoUrl = logoPath 
          ? `http://apiedepottest.gsotgroup.vn${logoPath}` 
          : ''; // Empty string instead of default Unsplash image
        ...
        return {
          ...
          address: address,
          image: logoUrl || 'https://images.unsplash.com/photo-1566576721346-d4a3b4eaeb55?w=800&q=80', // Keep default for image field for backward compatibility
          logo: logoUrl, // Empty if no logo from API

 ...
 * 
 * ⚠️ WARNING: HRMS_UserProfile API currently returns empty array (0 records)
 * This service is implemented for future use when API has data.
 * 
 * ALTERNATIVE SOLUTION: Extract unique NhaXeID from GetList_TaiXe_Thuoc_NhaXe API
 * which contains 26,000+ driver records with company information.
 */
  ...
  async fetchAllCompanies(): Promise<Company[]> {
    try {
      ...
      if (!rawData) {
        ...
        console.warn('⚠️ HRMS_UserProfile returned empty. Consider using driver data for company list.');
        return cached?.data || [];
      }
      ...
      // Log warning if no companies found
      if (companies.length === 0) {
        console.warn('⚠️ HRMS_UserProfile API returned 0 companies.');
        console.warn('💡 TIP: Use GetList_TaiXe_Thuoc_NhaXe API to extract NhaXeID for company list.');
      }

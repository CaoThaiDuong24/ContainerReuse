...
export const getDepots = async (req: Request, res: Response) => {
  try {
    console.log('📡 GET /api/iContainerHub_Depot - Starting...');
    const { province, search, status } = req.query;
    console.log('📝 Fetching depots from API service...');
    let depots = await depotApiService.fetchDepots();
    console.log(`✅ Fetched ${depots ? depots.length : 0} depots`);
    ...
    // Get container counts for each depot
    try {
      ...
      if (containers && Array.isArray(containers)) {
        // Count containers by depotId
        const containerCounts = containers.reduce((acc: any, container: any) => {
          const depotId = String(container.depotId);
          acc[depotId] = (acc[depotId] || 0) + 1;
          return acc;
        }, {});
        
        // Update containerCount for each depot
        depots = depots.map(depot => ({
          ...depot,
          containerCount: containerCounts[String(depot.id)] || 0
        }));
        
        console.log('✅ Updated container counts for depots');
      }
    } catch (containerError) {
      console.warn('⚠️ Failed to get container counts:', containerError);
      // Continue without container counts
    }
...
// DEPRECATED: Use /api/locations instead
export const getProvinces = async (_req: Request, res: Response) => {
  try {
    // Redirect to new location API
    return res.status(301).json({
      success: false,
      message: 'This endpoint is deprecated. Please use /api/locations instead',
      redirectTo: '/api/locations'
    });
  } catch (error) {
    return res.status(500).json({
      success: false,

  ...
  try {
    ...
    if (result.success) {
      ...
      try {
        ...
        // Use containerInfo if provided, otherwise use result.data
        const containerData = gateOutData.containerInfo || result.data;
...
// GET /api/containers/registered - Get registered containers for user (from local storage)
export const getRegisteredContainers = async (req: Request, res: Response) => {
  try {
    ...
    console.log('📋 Getting registered containers from local storage for user:', userId || 'all users');
    ...
    const shippingLineMap = new Map(shippingLines.map(line => [line.code, line]));
    ...
    const formattedContainers = registeredContainers.map((record) => {
      ...
      // Get shipping line code and find matching shipping line
      const shippingLineCode = gateOutData.HangTauID?.toString() || '';
      const shippingLineInfo = shippingLineMap.get(shippingLineCode);
      ...
      return {
        ...
        shippingLine: shippingLineInfo?.name || shippingLineCode || 'N/A',
        shippingLineLogo: shippingLineInfo?.logo || '',
        shippingLineColor: shippingLineInfo?.colorTemplate || '',
        emptyReturnDeadline: containerData.estimatedOutDate || containerData.HanTraRong || '',
...
export const getApiRegisteredContainers = async (req: Request, res: Response) => {
  try {
    ...
    console.log('📋 Getting registered containers from external API for user:', userId || 'all users');
    
    const { ContainerApiService } = require('../services/containerApiService.js');
    const containerApiService = new ContainerApiService();
    const containers = await containerApiService.getListDonHangReUseOutNow(userId);

...
export const getRegisteredContainers = async (req: Request, res: Response) => {
  try {
    // TODO: Implement logic to get containers registered by the user
    // For now, we'll use mock data based on gate-out containers
    const { ContainerApiService } = require('../services/containerApiService.js');
    const containerApiService = new ContainerApiService();
    
    // Get all containers and filter for registered ones
    // In a real implementation, this would query a database for user's registered containers
    const allContainers = await containerApiService.getListReUseNow();
    
    if (!allContainers || !Array.isArray(allContainers)) {
      return res.status(503).json({
        success: false,
        message: 'Unable to fetch container data',
        count: 0,
    ...
    // Mock: Return containers with status indicating they're registered
    // In production, you would filter based on actual user registration data
    const registeredContainers = allContainers.map((container: any) => ({
      ...container,
    ...
    return res.json({
      ...
      count: registeredContainers.length,
      data: registeredContainers
    });
  } catch (error) {

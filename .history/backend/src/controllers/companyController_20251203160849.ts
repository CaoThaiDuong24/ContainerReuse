...
/**
 * GET /api/companies/test-hrms-userprofile
 * Test endpoint to see raw HRMS_UserProfile API response
 */
export const testHRMSUserProfile = async (_req: Request, res: Response) => {
  try {
    console.log('🧪 Testing HRMS_UserProfile API...');
    
    // @ts-ignore - accessing private method for testing
    const rawData = await companyApiService.getUserProfileData();
    
    if (!rawData) {
      return res.status(503).json({
        success: false,
        message: 'Unable to fetch data from HRMS_UserProfile API',
        data: null
      });
    }
    
    // Return raw response to inspect structure
    return res.json({
      success: true,
      message: 'HRMS_UserProfile API raw response',
      dataCount: rawData.data ? rawData.data.length : 0,
      sampleRecord: rawData.data && rawData.data.length > 0 ? rawData.data[0] : null,
      allRecords: rawData.data || [],
      fullResponse: rawData
    });
  } catch (error) {
    console.error('❌ Error testing HRMS_UserProfile:', error);
    return res.status(500).json({
      success: false,
      message: 'Error testing HRMS_UserProfile API',
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
};
